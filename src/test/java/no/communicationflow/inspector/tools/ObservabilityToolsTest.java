...
class ObservabilityToolsTest {
	...
	void renderPrometheusTargets_includesHealthAndErrors() {
		...
		assertThat(rendered).contains("✅ api → http://api:8080/actuator/prometheus [health=up]")
			.contains(
					"❌ auth-service → http://host.docker.internal:9000/actuator/prometheus [health=down] lastError=connection refused");
	}
	...
	void renderPrometheusQuery_listsReturnedSamples() {
		...
		assertThat(rendered).contains("query: up{job=\"api\"}")
	...
	void renderPhase26ObservabilityStatus_reportsMissingAndPresentChecks() {
		...
		String rendered = ObservabilityTools.renderPhase26ObservabilityStatus(MAPPER.readTree(metricsJson),
				MAPPER.readTree(targetsJson), "http://localhost:8080", "http://localhost:9090");

		assertThat(rendered).contains("✅ grpc.server.requests present")
