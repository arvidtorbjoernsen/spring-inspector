 ...
 *
 * <p>Requires the API to be running with management endpoints exposed. A typical
 * {@code application.yml} for the api module includes:</p>
 * <pre>
 * management:
 *   endpoints:
 *     web:
 ...
 * </pre>
 *
 * <p>The base URL is configured via {@code inspector.actuator-url}
 * (env var {@code INSPECTOR_ACTUATOR_URL}), defaulting to
...
public class ActuatorTools {
            ...
            if (!components.isMissingNode() && components.isObject()) {
                components.properties().forEach(e -> {
                    ...
                    if (!details.isMissingNode()) {
                        details.properties().forEach(d ->
                                sb.append("    ").append(d.getKey()).append(": ").append(nodeText(d.getValue())).append("\n"));
                    }
                });
            }
            return sb.toString();
    ...
    public String conditions(
            @ToolParam(description = "Max number of negative matches to show (default 20).")
            Integer topN) {
        ...
        return get("/conditions", json -> {
            ...
            json.path("contexts").properties().forEach(ctx -> {
                ...
                sb.append("Negative matches: showing ").append(Math.min(limit, list.size()))
                  .append(" of ").append(list.size()).append("\n\n");
                list.stream().limit(limit).forEach(e -> {
                    sb.append("  ✗ ").append(e.getKey()).append("\n");
                    e.getValue().path("notMatched").forEach(cond ->
                            sb.append("      → ").append(field(cond, "condition"))
                              .append(": ").append(field(cond, "message")).append("\n"));
                });
            });
            return sb.toString();
        });
    ...
    public String beans(
            @ToolParam(description = """
                    ...
                    Examples: 'UserService', 'no.communicationflow', 'RestClient'.""")
            String filter) {
        return get("/beans", json -> {
            ...
            json.path("contexts").properties().forEach(ctx -> {
                ...
                if (filter == null || filter.isBlank()) {
                    sb.append("Context: ").append(ctx.getKey())
                      .append(" — ").append(beans.size()).append(" total beans\n")
                ...
                beans.properties().forEach(e -> {
                    if (e.getKey().toLowerCase().contains(f)
                            || field(e.getValue(), "type").toLowerCase().contains(f)) {
                        matching.add(e);
                    }
                });
                sb.append("Context: ").append(ctx.getKey())
                  .append(" — ").append(matching.size()).append(" matching '").append(filter).append("'\n\n");
                matching.forEach(e -> {
                    sb.append("  ").append(e.getKey()).append("\n");
                    ...
                    if (deps.isArray() && !deps.isEmpty()) {
                        List<String> depList = StreamSupport.stream(deps.spliterator(), false)
                                .map(JsonNode::asText).limit(5).collect(Collectors.toList());
                        if (deps.size() > 5) depList.add("...+" + (deps.size() - 5) + " more");
    ...
    public String mappings(
            @ToolParam(description = """
                    ...
                    Examples: '/api/v1/users', 'UserController', 'POST'. Leave empty to see all.""")
            String filter) {
            ...
            json.path("contexts").properties().forEach(ctx -> {
                ctx.getValue().path("mappings").properties().forEach(srv -> {
                    srv.getValue().forEach(mapping -> {
                        ...
                        if (filter == null || filter.isBlank()
                                || handler.toLowerCase().contains(filter.toLowerCase())
    ...
    public String env(
            @ToolParam(description = """
                    ...
                    Examples: 'datasource', 'spring.ai', 'server.port', 'INSPECTOR'.""")
            String keysFilter) {
        return get("/env", json -> {
            StringBuilder sb = new StringBuilder();
            ...
            json.path("propertySources").forEach(source -> {
                ...
                props.properties().forEach(e -> {
                    ...
                    if (keysFilter != null && !keysFilter.isBlank()
                            && !key.toLowerCase().contains(keysFilter.toLowerCase())) return;
    ...
    public String loggers(
            @ToolParam(description = """
                    ...
                    Leave empty to list all explicitly-configured loggers.""")
            String loggerName) {
        if (loggerName != null && !loggerName.isBlank()) {
            return get("/loggers/" + loggerName.trim(), json -> {
                ...
                return "Logger: %s\n  configured: %s\n  effective:  %s".formatted(
                        loggerName.trim(), configured, effective);
            });
        }
            ...
            json.path("loggers").properties().forEach(e -> {
                ...
                if (!"?".equals(level) && !"null".equalsIgnoreCase(level) && !level.isBlank()) {
                    sb.append("  ").append(e.getKey())
                      .append("  →  configured=").append(level)
                      .append("  effective=").append(field(e.getValue(), "effectiveLevel")).append("\n");
                }
            });
            return sb.toString();
        });
    ...
    public String setLogLevel(
            @ToolParam(description = "Fully-qualified logger name. Examples: 'no.communicationflow.api', 'org.springframework.security'.")
            String loggerName,
            @ToolParam(description = "Target log level: TRACE, DEBUG, INFO, WARN, ERROR, or OFF.")
            String level) {
        if (loggerName == null || loggerName.isBlank()) return "❌ loggerName is required.";
    ...
    public String metrics(
            @ToolParam(description = "Metric name (optional). Leave empty to list all metric names.")
            String metricName) {
        if (metricName != null && !metricName.isBlank()) {
            return get("/metrics/" + metricName.trim(), json -> {
                ...
                json.path("measurements").forEach(m ->
                        sb.append("  ").append(field(m, "statistic"))
                          .append(" = ").append(m.path("value").asDouble()).append("\n"));
                return sb.toString();
            });
        }
        return get("/metrics", json -> {
            ...
            return "Available metrics (%d):\n  %s\n\nCall metrics(name) for details.".formatted(
                    names.size(), String.join("\n  ", names));
        });
    }
    ...
    public String httpExchanges(
            @ToolParam(description = "Max number of recent exchanges to show (default 20).")
            Integer limit) {
        ...
        return get("/httpexchanges", json -> {
            ...
            exchanges.stream().skip(Math.max(0, exchanges.size() - n)).forEach(e -> {
                ...
                sb.append("  ").append(field(req, "method"))
                  .append(" ").append(field(req, "uri"))
                  .append(" → ").append(resp.path("status").asInt())
                  .append("  (").append(field(e, "timeTaken")).append(")\n");
            });
            return sb.toString();
        });
    }
    ...
    public String threadDump(
            @ToolParam(description = """
                    ...
                    Leave empty to see all RUNNABLE and BLOCKED threads.""")
            String nameFilter) {
        return get("/threaddump", json -> {
            ...
            json.path("threads").forEach(t -> {
                ...
                if (!nameMatch || !stateVisible) return;
                sb.append("[").append(state).append("] ").append(name).append("\n");
                t.path("stackTrace").forEach(f ->
                        sb.append("  at ").append(field(f, "className"))
                          .append(".").append(field(f, "methodName"))
                          .append(" (").append(field(f, "fileName"))
                          .append(":").append(f.path("lineNumber").asInt()).append(")\n"));
                sb.append("\n");
            });
    ...
    private String get(String path, JsonHandler handler) {
        try {
            String body = restClient.get().uri(path).retrieve()
                    ...
                    .body(String.class);
            if (body == null) return "Empty response from actuator at " + path;
    ...
    private static void appendTasks(StringBuilder sb, JsonNode tasks, String label, String intervalField) {
        if (!tasks.isMissingNode() && tasks.isArray() && !tasks.isEmpty()) {
            ...
            tasks.forEach(t -> {
                String target = t.path("runnable").path("target").asText(
                        t.path("target").asText("?"));
                String interval = t.path(intervalField).asText(
                        t.path("initialDelay").asText("?"));
